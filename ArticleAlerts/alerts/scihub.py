...
class SciHub(object):
    ...
    def __init__(self):
        ...
        self.available_base_url_list = ["https://sci-hub.im/"]
...
def main(download, file=None, search=None, search_download=None, limit=10, output=".", verbose=False, proxy=False):
    ...
    if download:
        ...
        if 'err' in result:
            ...
            logger.debug('Successfully downloaded file with identifier %s', download)
    elif search:
        results = sh.search(search, limit)
        if 'err' in results:
            logger.debug('%s', results['err'])
        else:
            logger.debug('Successfully completed search with query %s', search)
        print(results)
    elif search_download:
        results = sh.search(search_download, limit)
        if 'err' in results:
            logger.debug('%s', results['err'])
        else:
            logger.debug('Successfully completed search with query %s', search_download)
            for paper in results['papers']:
                result = sh.download(paper['url'], output)
                if 'err' in result:
                    logger.debug('%s', result['err'])
                else:
        ...
        with open(file, 'r') as f:
            ...
            for identifier in identifiers:
                result = sh.download(identifier, output)
                if 'err' in result:
                    logger.debug('%s', result['err'])
                else:
